...
I=1;
new='G:\Bistoon-Ph1AndIvector\prog_sh\bistoon_run\prog_dataamali\TestC5';
mkdir([new,'\Waves\'          ]);
...
[as,bs]=textread('list5.txt','%s %s');
for j=1:length(as)
     fid=fopen('res5.txt','a');

    TestPath=[as{j,1},'\',bs{j,1},'.wav'];
    
    %dos(['ALawConvert.exe ',TestPath,' ',[as{j,1},'\',bs{j,1},'_2.wav']]);
    %wavname=[bs{j,1},'_2.wav'];
    %delete(TestPath);
    %copyfile([as{j,1},'\',bs{j,1},'_2.wav'],TestPath);
    
    wavname=[bs{j,1},'.wav'];
    %dos(['SADLibTest.exe -Diarization ',TestPath,' ', [new,'\Diarization\'],' 0']);
    dos(['SpeakerDiarization.exe ',bs{j,1},' ',as{j,1},' ',[new,'\Diarization\'],' 1 0']);
    ...
    for i=1:length(SpkNames)
        ...
        if existFlag==2
            ...
            %dos(['SADLibTest.exe -SAD ',InputWaveFile,' ', OutputLabelFile,' ', Path.SADmodels,'   Temp 1 Energy.txt 0  10  10 0.0 3.80 50']);
            dos(['SAD.exe  ',InputWaveFile,' ', OutputLabelFile,' ', Path.SADmodels,'   Temp 1 Energy.txt 0  10  10 0.05 0.73 100']);
            %dos(['SAD.exe  ',InputWaveFile, ' ', OutputLabelFile,' ',Path.SADmodels,'   Temp 1 Energy.txt 0  10  10 0 3.8 50']);
            ...
            if sum(BB-AA)>5 %5 second                
                ...
                %dos(['MFCCWhitSAD.exe ',InputWaveFile,' ',[new,'\Diarization\',SpkNames{i,1},'.ftr'] ]);
                ...
                dos(['TestIvector4.exe CfgIvecSpk_2048_2.txt ',ftrpath,' ',lblpath,' ',modelpath,' ',TestName,' ',OutPath,' Test.lst UBM 1024 Thr10Spk.txt']);
...
F2=readHTK('G:\Bistoon-Ph1AndIvector\prog_sh\bistoon_run\prog_dataamali\TestC5\Diarization\A_S1.ftr',0);