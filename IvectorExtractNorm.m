...
function IvectorExtractNorm(pathstates,IvectorSvmpath,Norm_ivector,LDA_ivector,NAP_ivector,WCCN_ivector,LDAmodel,P_NAP,B_WCCN,c,g,kerneltype,TrainMethod,NumImpSpeak)
%------------------------------------------------------------------
load(pathstates);
...
for NS=1:TrainNorm.NumSpeaker
    k=0;
    %----------
    fid_List = fopen([JFA_s_lists  'FA-ivector'  ,'.lst'],'w');
    for j=1:length(TrainNorm.class)
        if TrainNorm.class(j)==NS
            k=k+1;
            VV = readHTK([IvectorSvmpath,'\Features\',TrainNorm.name{j},'.ftr'],0);
            save ([Path.Prog,'\data\Features\', TrainNorm.name{j},'.ascii'],'VV','-ASCII');
            Label4=TrainNorm.classname{j};
            fprintf(fid_List,'%s\n' ,[Label1 Label4  Label2 Label3 TrainNorm.name{j}]);
            SpeakerName=TrainNorm.classname{j};
            classSVM(k)=1;
        end
    end
    %----------
    ...
    %Train SVM
     ...
     SVMnew=['NormSVM_',num2str(NS),'.libsvm'];
     if (kerneltype=='2')
