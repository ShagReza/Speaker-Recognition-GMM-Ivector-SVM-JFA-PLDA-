...
function [LDAmodel,P_NAP,B_WCCN]=TrainIvector(pathstates,IvectorSvmpath,Norm_ivector,LDA_ivector,NAP_ivector,WCCN_ivector,NAPivector_Dim,c,g,SVM_Matlab,kerneltype,ny,TrainMethod)
%------------------------------------------------------------------
load(pathstates);
...
% Datapath=dir(Path.TargetSpeakers_Train);
% k=0;
% NumLan=length(Datapath)-2;
% for i=3:length(Datapath)
%     dirFiles=dir([Path.TargetSpeakers_Train,'\',Datapath(i).name,'\*.wav']);
%     for j=1:length(dirFiles)
%         dirFiles(j).name(end-3:end)=[];
%         VV = readHTK([IvectorSvmpath,'\Features\', dirFiles(j).name,'.ftr'],0);
%         save ([Path.Prog,'\data\Features\', dirFiles(j).name,'.ascii'],'VV','-ASCII');
%         k=k+1;
%         Label4=[Datapath(i).name,num2str(k)];
%         fprintf(fid_List,'%s\n' ,[Label1 Label4  Label2 Label3 dirFiles(j).name]);
%         class(k)=i-3;
%     end
% end
% fclose all;
